...
describe(`bootstrap`, () => {
  beforeEach(() => {
    ...
    enqueueExtractedPageComponent.mockClear()
  })
  ...
  it(`will queue query when page context is changed`, async () => {
    const service = getService({ isInBootstrap: false })
    service.send({ type: `PAGE_CONTEXT_MODIFIED` })
    // there is setTimeout in action handler for `CONTEXT_CHANGES`
    await sleep()
    expect(enqueueExtractedPageComponent).toBeCalledWith(testPath)
  })
})
